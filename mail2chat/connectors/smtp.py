...
class Connector(framework.BaseConnector):
    ...
    def submit(self, mail, **kwargs):
        ...
        try:
            ...
            client.close()
            self.log.debug("connector '{self}' successfully forwarded message to {proto}://{host}:{port}".format(
                    self=self,
    ...
    def pre_submit(self, mail, **kwargs):
        ...
        if "smtp_port" not in self.config.keys():
        ...
        if type(self.config.get("smtp_port")) != int or 1 > self.config.get("smtp_port") > 65535:
        ...
        if "smtp_use_tls" in self.config.keys():
            # Ensure 'smtp_use_tls' is a bool
            if type(self.config.get("smtp_use_tls")) != bool:
        ...
        if self.config.get("smtp_use_login", False):
            ...
            if type(self.config.get("smtp_login_password")) != str:
